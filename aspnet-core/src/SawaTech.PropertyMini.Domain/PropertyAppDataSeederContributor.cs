        ...
        public async Task SeedAsync(DataSeedContext context)
        {
            if (await _propertyFeatureRepository.GetCountAsync() <= 0)
            {
                await _propertyFeatureRepository.InsertAsync(
                    new Feature { Name = "Jacuzzi", IconUrl = "hhhhh" },
                    autoSave: true
                );

                await _propertyFeatureRepository.InsertAsync(
                    new Feature { Name = "Garage", IconUrl = "hhhhh" },
                    autoSave: true
                );

                await _propertyFeatureRepository.InsertAsync(
                    new Feature { Name = "Big Garden", IconUrl = "hhhhh" },
                    autoSave: true
                );

                await _propertyFeatureRepository.InsertAsync(
                    new Feature { Name = "Heater", IconUrl = "hhhhh" },
                    autoSave: true
                );
            }
        }
